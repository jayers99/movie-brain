...
- [ ] **Step 1: Write the failing tests** — a four-column row parses; a three-column row still yields `tt_listed=None`; an empty fourth cell yields `None`; **mixed arity in one file** (some rows with an id, some without) parses; `0004972`, `ttabc` and a trailing-space id each raise `ListFileError` (note `tt123` is ACCEPTED — spec §3's regex is `^tt\d+$` with no minimum digit count, and the real guard against a wrong-but-well-formed id is the resolver comparison, not the parser); the value round-trips through `upsert_list_entry` → `list_entries`; re-upserting a row **updates** `tt_listed` without clearing `film_id` (the existing guarantee must survive).
