...
| D18 | **One constant distance floor, `MAX_DISTANCE` 0.6; no slider, no top-N cap** | Owner decision 2026-09-06 (option 1 of 3): fewer controls. yt-brain's default; 7 films on the owner's query. Lives in `domain/search.py` beside the other thresholds; the hint carries the count so the user sees the floor working. *Superseded 2026-09-23 by `2026-09-23-search-recall-design.md` D3/D4.* |
...
- `web/app.py` — `create_app(repo, today=…, embedder=None)`; the app builds a `VectorIndex` when an embedder is given and passes it to `run_search`. `cli.py dashboard` passes a `SentenceTransformerEmbedder()` only when `SentenceTransformerEmbedder.available()` is true, so a missing extra yields no index and the install hint; a model that cannot load raises `SemanticUnavailable` on the first query, which `VectorIndex` latches for the rest of the process and `_semantic_stage` turns into the plain lexical result.
...
| Extra installed, `film_embedding` empty | `VectorIndex` is empty; stage 4 finds nothing to add or re-rank; result is Phase 1's. No hint — `embed` is the owner's manual step, and `status` reports the count, and the model is never loaded for an empty index |
