...
| O9 | **The `tier` column is stored but the UI exposes tier 1 only.** | Costs nothing now and spares a migration if tier 2 is ever ordered. Not a feature: no route or tab takes a tier. |
