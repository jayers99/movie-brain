...
| P5 | **Order mode takes a tier**, 1 or 2, as two tabs. *(Superseded 2026-09-15: every tier 1–5 is orderable, five numbered tabs — `ORDER_TIERS = (1, 2, 3, 4, 5)`, no other change.)* Rating (Phase B) stays the tier regardless of position; the order lives in the saved list only. | The owner chose to strict-order tiers 1 and 2 (~2,700 clicks) knowing the tier-2 order changes no rating. The `tier` column in the order tables was built for this; no schema change. |
