...
| M4 | **"Rank this" lights while the film awaits ordering.** `awaiting_order = tier in ORDER_TIERS and film not in rank_order(session, tier)`, computed on read, never stored. While awaiting, the button is pressed AND disabled, title `Waiting for Order tier N`. **Amended the same afternoon (M12/M13):** the button has four states — unplaced + unmarked (pool ticket), unplaced + marked (lit; click un-marks), placed + awaiting order (lit, disabled), placed + ordered (unlit; click RE-RANKS). Lit = `(marked and unplaced) or awaiting_order`. | One signal for "the ranker still owes this film a pass" — pool entry, re-ranking or re-ordering. Disabling while awaiting avoids the conflation where un-marking a marked-and-awaiting film would visibly un-press a button that should stay lit. |
...
| M12 | **"Rank this" on a placed film means "this ranking is wrong, ask me again."** (Owner ruling 2026-09-14, afternoon, after the mark had misled three times.) `rerank_film`: the placement, the film's own tiering verdicts and its tiering deferral in the open session are deleted, it leaves the order as a move does (M2), and the mark is SET. `_seed_new` never seeds a marked film, so a rated film is asked by comparison instead of landing back in its rating's tier. An anchor is refused; undo is cleared. | The move (M1) is the zero-click hand-set; this is the comparison path, two or three tiering clicks plus the ordering. The mark doubles as the pool ticket through the round trip, so a film the owner neither owns nor rated survives it. Verdicts naming the film as an ANCHOR stay in the log — only its own candidate trail is a search state. |
| M13 | **The mark comes off once served.** `clear_served_marks`, run on every rank-page read beside seed-on-read: a marked film that is placed in the open session and, for an ordered tier, inserted into its order, loses its mark. | The mark is a pending request, not a label. A film the owner neither owns nor rated thereby leaves the pool on service and keeps its tier, position and saved-list line, exactly as pool spec §4.6 rules for any film that leaves. The 22 marks the owner had already set were applied as re-rank requests at deploy (2026-09-14). |
...
- `POST /api/rank/rerank` body `{"film_id": int}` → 200 `{"film_id", "from_tier", "marked": true}`; 409 not placed / anchor; 404 no session (M12).
