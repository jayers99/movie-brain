...
class Repository:
    ...
    def merge_film(self, loser_id: int, survivor_id: int, today: date, note: str | None = None) -> MergeReport:
        ...
        with self._conn() as c:
            ...
            # log could cross the derived bounds). On the OTHER side a merged-away loser is
            # treated exactly like a removed film — every row naming it is DELETED, in every
            # session — because a re-point changes the verdict's INDEX whenever the two films
            # sat at different positions: order [S, D, E, L] and a candidate worse than D and
            # better than L re-points to better-than-S at index 0, crossing its own bounds, and
            # `order_step` raises on it forever. Each affected candidate pays at most log₂ k
            # re-probes on a rare merge and never gets a wrong order. A row MOVED by the
            # candidate loop above can still name the survivor on both sides (the loser's own
            # log held a verdict against the survivor); that audits nothing and is deleted.
            for row in c.execute(
            ...
            n = c.execute(
                "DELETE FROM rank_order_comparison WHERE other_film_id = ?", (loser_id,)
            ).rowcount
            if n:
                dropped["rank_order_comparison"] = dropped.get("rank_order_comparison", 0) + n
            n = c.execute(
